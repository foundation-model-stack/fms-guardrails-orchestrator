...
mod tests {
   ...
   async fn test_model_info() {
        ...
        let request = ModelInfoRequest {model_id: "test-model-1".to_string()};
        ...
        faux::when!(mock_client.model_info(request.clone())).once().then_return(Ok(expected_response.clone()));
        // Test the mock object's behaviour
        assert_eq!(mock_client.model_info(request).await.unwrap(), expected_response);
   }
