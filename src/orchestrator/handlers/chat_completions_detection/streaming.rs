...
use crate::{
    ...
    orchestrator::{
        common::{self, validate_detectors}, types::{ChatCompletionBatcher, ChatCompletionStream, ChatMessageIterator, ChoiceIndex, Chunk, DetectionBatchStream, DetectionStream, Detections}, Context, Error
    },
};
    ...
    tokio::spawn(
        async move {
            ...
            if output_detectors.is_empty() {
                ...
                let chat_completion_state: Arc<DashMap<ChoiceIndex, Vec<ChatCompletionChunk>>> = Arc::new(DashMap::new());
...
async fn handle_output_detection(
    ...
    task: &ChatCompletionsDetectionTask,
    detectors: HashMap<String, DetectorParams>,
    chat_completion_state: Arc<DashMap<ChoiceIndex, Vec<ChatCompletionChunk>>>,
    ...
    if detection_streams.len() == 1 {
        ...
        process_detection_stream(trace_id, chat_completion_state, detection_stream, response_tx).await;
    } else {
...
async fn handle_whole_doc_output_detection(
    ...
    _chat_completion_state: Arc<DashMap<ChoiceIndex, Vec<ChatCompletionChunk>>>,
...
/// and updates shared chat completions.
    ...
    while let Some((message_index, result)) = chat_completion_stream.next().await {
        match result {
            Ok(Some(chat_completion)) => {
                ...
                let input_tx = input_txs.get(&choice_index).unwrap();
                let _ = input_tx.send(Ok((message_index, choice_text))).await;

                // Update chat completion state
                match chat_completion_state.entry(choice_index) {
                    dashmap::Entry::Occupied(mut entry) => {
                        entry.get_mut().push(chat_completion);
                    },
                    dashmap::Entry::Vacant(entry) => {
                        entry.insert(vec![chat_completion]);
                    },
                }
            }
            Ok(None) => (),
...
fn output_detection_response(
    ...
    //     .unwrap_or_default();
    // let mut chat_completion = chat_completions.last().cloned().unwrap();
    let mut chat_completion = chat_completions
        .get(chunk.input_end_index)
        .cloned()
        .unwrap_or_default();
    chat_completion.choices = vec![ChatCompletionChunkChoice { 
        index: choice_index, 
...
async fn process_detection_stream(
    ...
    while let Some(result) = detection_stream.next().await {
        match result {
            Ok((choice_index, _detector_id, chunk, detections)) => {
...
async fn process_detection_batch_stream(
    ...
    while let Some(result) = detection_batch_stream.next().await {
        match result {
            Ok((chunk, choice_index, detections)) => {
                let chat_completion = output_detection_response(&chat_completion_state, choice_index, chunk, detections).unwrap();
                // Send message to response channel
                if response_tx.send(Ok(Some(chat_completion))).await.is_err() {
                    info!(%trace_id, "task completed: client disconnected");
