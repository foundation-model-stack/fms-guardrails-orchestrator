...
use common::{
    chunker::CHUNKER_UNARY_ENDPOINT, detectors::{
        ...
        TestOrchestratorServer, ORCHESTRATOR_CONFIG_FILE_PATH, ORCHESTRATOR_INTERNAL_SERVER_ERROR_MESSAGE, ORCHESTRATOR_UNARY_ENDPOINT, ORCHESTRATOR_UNSUITABLE_INPUT_MESSAGE
    }
};
...
async fn no_detections() -> Result<(), anyhow::Error> {
    ...
    // Assertions on no output detections scenario
    assert!(response.status() == StatusCode::OK);
    let results = response.json::<ClassifiedGeneratedTextResult>().await?;
...
async fn input_detector_detections() -> Result<(), anyhow::Error> {
    ...
    let tokenization_results = vec![Token {
        start: 0,
        end: 40,
    ...
    let mock_tokenization_responses = vec![TokenizationResults {
        results: Vec::new(),
        token_count: 61,
    }, TokenizationResults {
        results: Vec::new(),
    ...
    generation_mocks.insert(
        ...
        Mock::new(
            ...
            MockResponse::pb(mock_tokenization_responses[0].clone()),
        ),
    );
    ...
    generation_mocks.insert(
        ...
        Mock::new(
            ...
            .with_headers(gen_headers.clone()),
            MockResponse::pb(mock_tokenization_responses[1].clone()),
        ),
    );
    ...
    chunker_mocks.insert(
        ...
        Mock::new(
            ...
            .with_headers(chunker_headers.clone()),
            MockResponse::pb(TokenizationResults {
                results: vec![tokenization_results[0].clone(), tokenization_results[1].clone()],
                token_count: 0,
            }),
        ),
    );
...
async fn output_detector_detections() -> Result<(), anyhow::Error> {
    ...
    generation_mocks.insert(
        ...
        Mock::new(
            ...
            .with_headers(gen_headers.clone()),
            MockResponse::pb(mock_generation_responses[0].clone()),
        ),
    );
    ...
    generation_mocks.insert(
        ...
        Mock::new(
            ...
            .with_headers(gen_headers.clone()),
            MockResponse::pb(mock_generation_responses[1].clone()),
        ),
    );
...
async fn output_detector_client_error() -> Result<(), anyhow::Error> {
    ...
    generation_mocks.insert(
        ...
        Mock::new(
            ...
            .with_headers(gen_headers.clone()),
            MockResponse::pb(GeneratedTextResult {
