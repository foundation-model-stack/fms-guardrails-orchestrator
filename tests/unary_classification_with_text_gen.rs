...
async fn no_detections() -> Result<(), anyhow::Error> {
    ...
    detector_mocks.mock(|when, then| {
        when.post()
            ...
            .json(ContentAnalysisRequest {
                contents: vec![
                    expected_response.generated_text.clone()
                ],
                detector_params: DetectorParams::new(),
            });
        then.json(vec![Vec::<ContentAnalysisResponse>::new(),]);
...
async fn input_detector_client_error() -> Result<(), anyhow::Error> {
    ...
    detector_mocks.mock(|when, then| {
        ...
        then.internal_server_error().message(expected_detector_error.message);
    });
...
async fn output_detector_client_error() -> Result<(), anyhow::Error> {
    ...
    detector_mocks.mock(|when, then| {
        ...
        then.json([
            vec![[Vec::<ContentAnalysisResponse>::new()]],
        ]);
    });
